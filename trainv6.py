...
class SRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return lr, hr, self.paths[idx]  # include image path for filenames
...
def main():
    ...
    for epoch in tqdm(range(total_epochs), desc="Epochs", unit="epoch"):
        ...
        for i, (lr, hr, img_paths) in batch_pbar:
            ...
            # Save first batch inputs for visualization on CPU
            if i == 0:
                first_lr = lr.cpu()
                first_hr = hr.cpu()
                first_paths = img_paths
            ...
            if i == 0:
                first_sr = gen_hr.cpu()  # Save generated HR from first batch for visual

        batch_pbar.close()

        # Save 5 sample images per epoch with HR, LR, SR and PSNR in filename
